...
void myRenderScene() {
	...
	float trianglesVerts[] = {
		...
		0.7f, -0.42f, 0.0f,		dis(gen), dis(gen), dis(gen),
		0.7f, -0.18f, 0.0f,		dis(gen), dis(gen), dis(gen),
		-0.7f, -0.3f, 0.5f,		dis(gen), dis(gen), dis(gen),

		-0.25f, 0.7f, 0.0f,		dis(gen), dis(gen), dis(gen),
		-0.40f, 0.55f, 0.0f,	dis(gen), dis(gen), dis(gen),
		0.5f, -0.6f, 0.5f,		dis(gen), dis(gen), dis(gen),

		-0.57f, -0.53f, 0.0f,	dis(gen), dis(gen), dis(gen),
		-0.43f, -0.67f, 0.0f,	dis(gen), dis(gen), dis(gen),
		0.32f, 0.62f, 0.5f,		dis(gen), dis(gen), dis(gen),
	};
	...
	case 1:
		// Draw separate lines:
		glBindVertexArray(myVAO[iLines]);
		glVertexAttrib3f(aColor_loc, dis(gen), dis(gen), dis(gen));		// Brown color (R, G, B values).
		glDrawArrays(GL_LINES, 0, 6);
		break;
	case 2:
		// Draw line strip:
		glBindVertexArray(myVAO[iLines]);
		glVertexAttrib3f(aColor_loc, dis(gen), dis(gen), dis(gen));		// Magenta color (R, G, B values).
		glDrawArrays(GL_LINE_STRIP, 0, 6);
		break;
	case 3:
		// Draw line loop:
		glBindVertexArray(myVAO[iLines]);
		glVertexAttrib3f(aColor_loc, dis(gen), dis(gen), dis(gen));		// Cyan color (R, G, B values).
		glDrawArrays(GL_LINE_LOOP, 0, 6);
		break;
	case 4:
		// Draw three points
		glBindVertexArray(myVAO[iPoints]);
		glVertexAttrib3f(aColor_loc, dis(gen), dis(gen), dis(gen));		// An orange-red color (R, G, B values).
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (key == GLFW_KEY_ESCAPE || key == GLFW_KEY_Q) {
		...
		CurrentMode = (CurrentMode+2) % 5;	// Takes on values from 0 to 4
	}
}
...
int main() {
	...
	printf("Press ESCAPE or 'Q' or 'q' to quit.\n");
